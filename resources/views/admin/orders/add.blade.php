...
@section('content')
    <div class="card mb-3">
        <div class="card-header">
            <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                    <h5 class="mb-0" data-anchor="data-anchor">Oders :: Order Add </h5>
                </div>
        ...
        <div class="card-body">
            <form class="row" id="add" method="POST" action="{{ route('admin.orders.add') }}"
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <input class="form-control" id="service_date" placeholder="Enter Date" name="service_date"
                        type="date" value="{{ old('service_date') }}" />
                    @error('service_date')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <input class="form-control" id="service_time" placeholder="Enter Date" name="service_time"
                        type="time" value="{{ old('service_time') }}" />
                    @error('service_time')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="payment_type" class="form-select" id="payment_type">
                        <option value="1" @selected(old('payment_type', 1) == 1)> Online </option>
                        <option value="0" @selected(old('payment_type', 1) == 0)> Cash </option>
                    </select>
                    @error('payment_type')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="payment_status" class="form-select" id="payment_status">
                        <option value="1" @selected(old('payment_status', 1) == 1)> Paid </option>
                        <option value="2" @selected(old('payment_status', 1) == 2)> Failed </option>
                        <option value="0" @selected(old('payment_status', 1) == 0)> Pending </option>
                    </select>
                    @error('payment_type')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="status" class="form-select" id="status">
                        <option value="1" @selected(old('status', 1) == 1)> Completed </option>
                        <option value="2" @selected(old('status', 1) == 2)> Cancelled </option>
                        <option value="0" @selected(old('status', 1) == 0)> Pending </option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change', '#user_id', function() {
                ...
                var address_id = {{ old('address_id', 0) }};
                $.ajax({
        ...
        $("#add").validate({
            ...
            errorPlacement: function(error, element) {
                if (element.attr("name") == "user_id"||element.attr("name") == "plan_id" || element.attr("name") == "vehicle_id") {
