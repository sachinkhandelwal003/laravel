    ...
    <div class="card mb-3">
        <div class="card-header">
            <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                    <h5 class="mb-0" data-anchor="data-anchor">Service Days :: Service Day Edit </h5>
                </div>
        ...
        <div class="card-body">
            <form class="row" id="edit" method="POST"
                action="{{ route('admin.service-days.edit', $service_day['id']) }}" enctype='multipart/form-data'>
                @csrf
                    ...
                    <select name="cleaner_id" class="form-select" id="cleaner_id">
                        ...
                        @if ($cleaners)
                            @foreach ($cleaners as $key => $cleaner)
                                <option value="{{ $cleaner['id'] }}" @selected(old('cleaner_id', $service_day['cleaner_id']) == $cleaner['id'])>
                                    {{ $cleaner['name'] }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <select name="order_id" class="form-select" id="order_id">
                        ...
                        @if ($orders)
                            @foreach ($orders as $key => $order)
                                <option value="{{ $order['id'] }}" @selected(old('order_id', $service_day['order_id']) == $order['id'])>
                                    {{ $order['order_number'] }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <input name="date" class="form-control" id="date" type="date"
                        value="{{ old('date', $service_day['date']) }}" />
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="is_full_cleaning" class="form-select" id="is_full_cleaning">
                        <option value="1" @selected(old('is_full_cleaning', $service_day['is_full_cleaning']) == 1)> Yes </option>
                        <option value="0" @selected(old('is_full_cleaning', $service_day['is_full_cleaning']) == 0)> No </option>
                    </select>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="status" class="form-select" id="status">
                        <option value="0" @selected(old('status', $service_day['status']) == 0)> Pending </option>
                        <option value="1" @selected(old('status', $service_day['status']) == 1)> Completed </option>
                        <option value="2" @selected(old('status', $service_day['status']) == 2)> Not Completed </option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
