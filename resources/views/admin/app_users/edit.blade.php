...
@section('content')
    <div class="card mb-3">
        <div class="card-header">
            <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                    <h5 class="mb-0" data-anchor="data-anchor">Users :: User Edit </h5>
                </div>
        ...
        <div class="card-body">
            <form class="row" id="edit" method="POST" action="{{ route('admin.app-users.edit', $app_users['id']) }}"
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <input class="form-control" id="email" placeholder="Enter Email" name="email" type="email"
                        value="{{ old('email', $app_users['email']) }}" />
                    @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <input class="form-control" id="phone" placeholder="Enter Mobile" name="phone" type="text"
                        value="{{ old('phone', $app_users['phone']) }}" />
                    @error('phone')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="status" class="form-select" id="status">
                        <option value="1" @selected(old('status', $app_users['status']) == 1)> Active </option>
                        <option value="0" @selected(old('status', $app_users['status']) == 0)> Inactive </option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
