    ...
    <div class="card mb-3">
        <div class="card-header">
            <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                    <h5 class="mb-0" data-anchor="data-anchor">Transactions :: Transaction Edit </h5>
                </div>
        ...
        <div class="card-body">
            <form class="row" id="edit" method="POST"
                action="{{ route('admin.transactions.edit', $transaction['id']) }}" enctype='multipart/form-data'>
                @csrf
                    ...
                    <select name="cleaner_id" class="form-select" id="cleaner_id">
                        ...
                        @if ($cleaners)
                            @foreach ($cleaners as $key => $cleaner)
                                <option value="{{ $cleaner['id'] }}" @selected(old('cleaner_id,', $transaction['cleaner_id']) == $cleaner['id'])>
                                    {{ $cleaner['name'] }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <input name="amount" class="form-control" id="amount" type="number"
                        value="{{ old('amount', $transaction['amount']) }}" min="0" />
                ...
                <div class="col-lg-6 mt-2">
                    ...
                    <select name="type" class="form-select" id="type">
                        <option value="1" @selected(old('type', $transaction['type']) == 1)> Paid </option>
                        <option value="2" @selected(old('type', $transaction['type']) == 2)> Received </option>
                    </select>
                    @error('type')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
